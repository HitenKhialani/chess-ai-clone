...
import { Brain, Upload, Crown, Zap, TrendingUp, AlertCircle, Search, Database, Volume2, BarChart3, Lock } from "lucide-react"
...
import DeepSeekAnalysis from '@/components/DeepSeekAnalysis'
...
export default function AnalysisPage() {
  ...
  function onDrop(sourceSquare: string, targetSquare: string) {
    ...
    try {
      ...
      const piece = tempGameForMoveValidation.get(sourceSquare as any);
      const isPromotion = piece && 
                         piece.type === 'p' && 
                         ((piece.color === 'w' && targetSquare[1] === '8') || 
                          (piece.color === 'b' && targetSquare[1] === '1'));
      if (isPromotion) {
        moveObj.promotion = 'q'; // Default to queen for analysis
      }
      const move = tempGameForMoveValidation.move(moveObj)
  ...
  const handleGMSelect = async (gmDisplayName: string) => {
    ...
    try {
      ...
      if (typeof window !== 'undefined') {
        ...
        const moveInput: any = { from: bestMove.slice(0,2), to: bestMove.slice(2,4) };
      // Check if promotion is needed for engine move
      const moves = chess.moves({ verbose: true });
      const promotionMove = moves.find(m => m.from === moveInput.from && m.to === moveInput.to && m.promotion);
      if (promotionMove) {
        moveInput.promotion = 'q'; // Default to queen for engine moves
      }
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10 relative z-10 flex flex-col items-center justify-center">
        <div className="text-center mb-10">
          <div className="flex items-center justify-center mb-4">
            <div className="p-3 rounded-full bg-gradient-to-r from-[#00F5D4]/20 to-[#57CC99]/20 backdrop-blur-sm border border-[#00F5D4]/30">
              							<BarChart3 className="w-8 h-8 text-[#00F5D4]" />
            </div>
          </div>
          <h1 className="text-5xl font-bold mb-4 text-[var(--primary-text)]">Game Analysis</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-stretch justify-center">
          <div className="h-full">
            <Card className="krishna-card border-rgb(153, 0, 255)/30 card-shadow text-[var(--card-foreground)] purple-glow h-[700px] max-h-[80vh] flex flex-col transition-transform duration-300 ease-out hover:-translate-y-1 hover:shadow-2xl">
              <CardHeader>
              ...
              <CardContent className="flex-1 flex flex-col justify-between">
                ...
                <div className="flex flex-col lg:flex-row justify-center gap-8 mt-4">
                  <div className="flex gap-2 justify-center w-full">
                                <Button onClick={handleUndo} disabled={moveStack.length === 0} className="bg-[var(--accent)] text-[var(--card-foreground)] border border-[var(--border)] hover:bg-[#B5532A]">Undo</Button>
                    <Button className="bg-[var(--accent)] text-[var(--card-foreground)] border border-[var(--border)] hover:bg-[#B5532A] btn-glow" onClick={resetAnalysis}>Reset Board</Button>
                  </div>
                </div>
              </CardContent>
            </Card>
          ...
          <div className="h-full">
            <Card className="card bg-[var(--card)] border-[var(--border)] shadow-lg text-[var(--primary-text)] h-full transition-transform duration-300 ease-out hover:-translate-y-1 hover:shadow-2xl">
              <CardHeader>
                    ...
                    <div className="flex gap-2">
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <Button variant="outline" className="w-full card border border-[var(--accent)] text-[var(--primary-text)] hover:bg-[var(--accent)] transition-transform duration-200 hover:-translate-y-0.5">
                        ...
                        <DropdownMenuContent className="w-full bg-[var(--card)] border border-[var(--accent)] text-[var(--primary-text)] shadow-lg">
                          {savedPgns.length > 0 ? (
                            savedPgns.map((p) => (
                              <DropdownMenuItem key={p.id} onSelect={() => handleLoadPgn(p.id)} className="text-[var(--primary-text)] hover:bg-[var(--accent)] transition-colors">
          ...
          {/* Grandmaster Analysis Card - Matches Other Cards */}
          {(selectedGM || explanationText) && (
            <div className="lg:col-span-2">
              <Card className="card bg-[var(--card)] border-[var(--border)] shadow-lg text-[var(--primary-text)] h-[700px] max-h-[80vh] flex flex-col transition-transform duration-300 ease-out hover:-translate-y-1 hover:shadow-2xl">
              <CardHeader>
                ...
                <CardDescription className="text-[var(--secondary-text)]">
                  {selectedGM ? `${selectedGM}'s analysis and insights` : 'Analysis from chess masters'}
                </CardDescription>
              </CardHeader>
              <CardContent className="flex-1 flex flex-col">
                {/* Grandmaster Name and Voice Toggle */}
                {selectedGM && (
                  <div className="mb-4">
                ...
                {/* Chatbot UI */}
                {chatbotActive && chatMessages.length > 0 && (
                  <div className="mb-4 card rounded-lg p-4 border border-[var(--accent)]">
                    <div className="font-semibold text-[var(--accent)] mb-2">{selectedGM}:</div>
                    <div className="text-[var(--primary-text)]">
                      <span className="block text-sm" dangerouslySetInnerHTML={{ __html: chatMessages[chatMessages.length - 1].text }} />
                    </div>
                  </div>
                )}

                {/* Suggested Move Analysis */}
                {explanationText && (
          ...
          <div className="lg:col-span-2">
            <DeepSeekAnalysis moves={moveStack} pgn={pgn} />
          </div>
        </div>
      </div>
    </main>
