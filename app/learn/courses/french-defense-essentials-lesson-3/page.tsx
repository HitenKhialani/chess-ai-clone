...
const moveExplanations = [
  '1.e4 e6 2.d4 d5: French structure.',
  '3.Nc3 Bb4: The classic pin.'
];

export default function WinawerVariationLesson() {
    ...
    <main className="min-h-screen bg-[var(--background)] text-[var(--primary-text)]">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-purple-600 text-[var(--card-foreground)] p-4 shadow-lg">
        <div className="max-w-7xl mx-auto flex items-center">
          <Link 
            href="/learn/courses/french-defense-essentials" 
      ...
      <div className="bg-gradient-to-r from-blue-600 to-purple-600 text-[var(--card-foreground)] p-6">
        <div className="max-w-7xl mx-auto">
          <h1 className="text-3xl font-bold mb-2">Lesson 3: Winawer Variation – Double-Edged Battles</h1>
          <p className="text-blue-100">Sharp, double-edged positions with open lines and structural imbalances.</p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 lg:px-6">
        ...
        <div className="min-h-[60vh] lg:min-h-[70vh] flex flex-col lg:flex-row gap-4 lg:gap-6 p-4 lg:p-6">
          ...
          <div className="w-full lg:w-3/5 bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-950/30 dark:to-emerald-950/30 rounded-2xl p-6 border border-[var(--border)] dark:border-green-800 shadow-xl hover:shadow-2xl transition-all duration-300 flex flex-col justify-center items-center">
            <h3 className="text-xl font-bold mb-4 text-[var(--accent)] dark:text-green-300 text-center">Interactive Chess Board</h3>
            ...
            <div className="text-center mt-3">
              <div className="bg-[var(--card)] dark:bg-[var(--card)] rounded-lg px-4 py-2 shadow-md">
                <span className="text-sm font-semibold text-[var(--accent)] dark:text-green-300">Move {Math.floor(moveIdx/2)} of {Math.floor(moves.length/2)}</span>
              </div>
            </div>
          </div>
          ...
          <div className="w-full lg:w-2/5 bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-950/30 dark:to-indigo-950/30 rounded-2xl p-6 border border-[var(--border)] dark:border-blue-800 shadow-xl hover:shadow-2xl transition-all duration-300 flex flex-col">
            <div className="flex items-center mb-4">
              ...
              <div>
                <h2 className="text-xl font-bold text-[var(--primary)] dark:text-blue-300">Winawer Variation</h2>
                <div className="flex gap-2 mt-1">
                  <span className="bg-[var(--accent)] text-[var(--card-foreground)] px-2 py-1 rounded text-xs font-bold shadow-md">Advanced</span>
                  <span className="bg-[var(--primary)] text-[var(--card-foreground)] px-2 py-1 rounded text-xs font-bold shadow-md">Double-Edged</span>
                </div>
              </div>
            </div>
            <div className="bg-[var(--card)] dark:bg-[var(--card)] rounded-xl p-4 mb-4 shadow-lg">
              <h3 className="font-semibold text-base mb-3 text-[var(--card-foreground)] dark:text-gray-200">📚 Fundamentals</h3>
              <p className="text-[var(--card-foreground)] dark:text-[var(--muted-foreground)] text-sm leading-relaxed mb-3">The Winawer leads to sharp, double-edged play. Handle imbalances, use open lines, and calculate precisely.</p>
            </div>
            ...
            <div className="bg-gradient-to-r from-purple-100 to-pink-100 dark:from-purple-900/30 dark:to-pink-900/30 rounded-xl p-4 shadow-lg">
              <h4 className="font-semibold text-purple-800 dark:text-purple-200 mb-2 text-sm">🎯 Current Position</h4>
              {moveIdx > 0 && moveIdx <= moveExplanations.length && (<p className="text-sm text-[var(--accent)] dark:text-purple-300 leading-relaxed">{moveExplanations[moveIdx - 1]}</p>)}
              {moveIdx === 0 && (<p className="text-sm text-[var(--accent)] dark:text-purple-300">Starting position - White to move</p>)}
            </div>
          </div>
        </div>
          ...
          <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-thin scrollbar-thumb-purple-300 scrollbar-track-purple-100 dark:scrollbar-thumb-purple-600 dark:scrollbar-track-purple-900">
            {moveExplanations.map((explanation, index) => (
              <div
                ...
                className={`flex-shrink-0 w-72 lg:w-80 xl:w-96 bg-[var(--card)] dark:bg-[var(--card)] rounded-xl p-4 cursor-pointer transition-all duration-300 hover:shadow-xl transform hover:scale-105 border-2 ${index === moveIdx - 1 ? 'ring-2 ring-blue-500 shadow-xl border-blue-300' : 'hover:ring-1 hover:ring-purple-300 border-transparent hover:border-[var(--border)]'}`}
              >
                <div className="flex items-center mb-3">
                  <div className={`${index === moveIdx - 1 ? 'bg-[var(--primary)]' : 'bg-[var(--accent)]'} w-10 h-10 rounded-full flex items-center justify-center text-[var(--accent-foreground)] font-bold mr-3 shadow-lg`}>{index + 1}</div>
                  <h4 className={`${index === moveIdx - 1 ? 'text-[var(--primary)] dark:text-blue-300' : 'text-[var(--card-foreground)] dark:text-[var(--muted-foreground)]'} font-semibold`}>Move {index + 1}</h4>
                </div>
                <p className="text-sm text-[var(--muted-foreground)] dark:text-[var(--muted-foreground)] mb-3 leading-relaxed">{explanation}</p>
                <div className="flex justify-between items-center">
                  <span className="text-xs text-[var(--accent)] dark:text-purple-400 font-medium bg-[var(--secondary)] dark:bg-purple-900 px-2 py-1 rounded">{moves[index * 2]} {moves[index * 2 + 1]}</span>
                  <div className="flex gap-1"><div className="w-2 h-2 bg-blue-400 rounded-full shadow-sm"></div><div className="w-2 h-2 bg-green-400 rounded-full shadow-sm"></div><div className="w-2 h-2 bg-purple-400 rounded-full shadow-sm"></div></div>
                </div>
              </div>
            ))}
          </div>
