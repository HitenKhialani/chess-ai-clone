          ...
          <div className="card border border-[var(--shadow)] shadow-lg p-6 flex flex-col items-center">
            ...
            <div className="flex gap-2 mt-4 w-full flex-wrap">
              {moveNotations.map((notation, idx) => (
                <span
                  key={idx}
                  className={`bg-[var(--card)] border border-[var(--accent)] text-[var(--primary-text)] px-3 py-1 rounded mb-2 ${selectedMoveIdx === idx ? "bg-[var(--primary)] text-[var(--accent-foreground)] border-blue-400" : "bg-black/30 text-purple-100 border-transparent hover:bg-black/50"}`}
