...
export default function BotPlayPage() {
  ...
  // Dynamic accuracy and ELO calculation
  const [dynamicAccuracy, setDynamicAccuracy] = useState<number>(0);
  const [dynamicElo, setDynamicElo] = useState<string>('0');
  ...
  // --- STATE FOR BOT MOVE HIGHLIGHT ---
  const [lastBotMove, setLastBotMove] = useState<{ from: string; to: string } | null>(null);

  // --- STATE FOR USER MOVE HIGHLIGHT ---
  const [lastUserMove, setLastUserMove] = useState<{ from: string; to: string } | null>(null);
  ...
  useEffect(() => {
    ...
    // Update draw state whenever game state changes
    const newDrawState = { ...drawState };
    updateDrawHistory(newGame, newDrawState);
    setDrawState(newDrawState);

    // Highlight user's last move if it's user's turn and a new user move was made
    if (newGame.turn() === 'b' && moveStack.length > 0) {
      ...
      if (lastPair && lastPair.userMove && !lastPair.aiMove) {
        // Find from/to squares for user's move
        const prevGame = new Chess(getFenFromMoveStack(moveStack.slice(0, -1)));
        const legalMoves = prevGame.moves({ verbose: true });
        const found = legalMoves.find(m => m.san === lastPair.userMove);
        if (found) {
          setLastUserMove({ from: found.from, to: found.to });
        } else {
          setLastUserMove(null);
        }
      }
    }
    // Clear user highlight when bot moves
    if (newGame.turn() === 'w') {
      setLastUserMove(null);
    }
        ...
        setTimeout(() => {
          makeEngineMove(newGame.fen(), (aiMoveSan) => {
            if (aiMoveSan) { // Only update if AI move was successful
              // Get the move's from and to squares for highlight
              const tempGame = new Chess(newGame.fen());
              const legalMoves = tempGame.moves({ verbose: true });
              const found = legalMoves.find(m => m.san === aiMoveSan);
              if (found) {
                setLastBotMove({ from: found.from, to: found.to });
              } else {
                setLastBotMove(null);
              }
              ...
              // Do NOT clear highlight on a timer; keep it until user moves
            } else {
              setLastBotMove(null);
              console.error("AI move failed, not updating moveStack.");
            }
          });
        }, 900); // Increased delay for bot move (900ms)
      }
    }
  ...
  const makeEngineMove = (fen: string, onAIMoveSan: (san: string) => void) => {
    ...
    const handler = (event: MessageEvent) => {
      ...
      if (line.startsWith('bestmove')) {
        ...
        if (isLegal) {
          // Handle promotion moves (5 characters: e.g., "g2g1q")
          const moveObj: any = { from: best.slice(0, 2), to: best.slice(2, 4) };
          if (best.length === 5) {
            // Intelligent AI promotion choice
            const smartPromotion = getSmartPromotionChoice(tempGame, best.slice(0, 2), best.slice(2, 4), best.slice(4, 5));
            moveObj.promotion = smartPromotion;
          }
  ...
  const handleSquareClick = (square: Square) => {
    if (selectedSquare) {
      ...
      const movingPiece = tempGameForMove.get(selectedSquare);
      ...
      if (isPromotion) {
        // Show promotion dialog for user choice
      ...
      const moveObj: any = { from: selectedSquare, to: square };
      const move = tempGameForMove.move(moveObj);
      if (move) {
        ...
        setSelectedSquare(null);
        setLegalMoves([]);
        setLastBotMove(null); // Clear bot move highlight when user plays
        // Set user move highlight
        setLastUserMove({ from: selectedSquare, to: square });
      } else {
        setSelectedSquare(null);
        setLegalMoves([]);
      }
  ...
  const onDrop = (src: string, dst: string, piece: any) => {
    ...
    // Check if this is a pawn promotion move
    const movingPiece = tempGameForMove.get(src as any);
    const isPromotion = movingPiece && 
                       movingPiece.type === 'p' && 
                       ((movingPiece.color === 'w' && dst[1] === '8') || 
                        (movingPiece.color === 'b' && dst[1] === '1'));
    
    if (isPromotion) {
      // Show promotion dialog for user choice
      setPendingPromotion({ from: src, to: dst, color: game.turn() });
      return false;
    }

    // Normal move (no promotion)
    const moveObj: any = { from: src, to: dst };
    const move = tempGameForMove.move(moveObj);
    if (!move) {
      return false;
    }
    ...
    // Update draw state
    const newDrawState = { ...drawState };
    updateDrawHistory(tempGameForMove, newDrawState);
    setDrawState(newDrawState);
    ...
    setLastBotMove(null); // Clear bot move highlight when user plays
    // Set user move highlight
    setLastUserMove({ from: src, to: dst });
    return true;
  };

  // --- SMART AI PROMOTION LOGIC ---
  const getSmartPromotionChoice = (game: Chess, from: string, to: string, engineChoice: string): string => {
    // Start with engine's choice (usually queen)
    let choice = engineChoice;
    
    try {
      // Check for specific tactical situations where under-promotion is better
      
      // 1. Stalemate avoidance - if queen would cause stalemate, try other pieces
      const testGame = new Chess(game.fen());
      testGame.move({ from, to, promotion: 'q' });
      if (testGame.isStalemate()) {
        console.log('🤖 AI: Avoiding stalemate, choosing rook instead of queen');
        return 'r'; // Rook is often safe alternative
      }
      
      // 2. Knight fork opportunities - check if knight can fork king and other piece
      const knightGame = new Chess(game.fen());
      knightGame.move({ from, to, promotion: 'n' });
      const knightMoves = knightGame.moves({ verbose: true });
      const forkMoves = knightMoves.filter(move => {
        const testFork = new Chess(knightGame.fen());
        testFork.move(move);
        return testFork.isCheck(); // Knight checking = potential fork
      });
      
      if (forkMoves.length > 0 && Math.random() < 0.3) { // 30% chance to choose tactical knight
        console.log('🤖 AI: Knight fork opportunity detected!');
        return 'n';
      }
      
      // 3. Endgame considerations - in endgames, sometimes rook is better
      const pieces = game.board().flat().filter(p => p !== null);
      const isEndgame = pieces.length <= 8; // 8 or fewer pieces = endgame
      
      if (isEndgame && Math.random() < 0.15) { // 15% chance in endgames
        const endgamePieces = ['r', 'b', 'n'];
        choice = endgamePieces[Math.floor(Math.random() * endgamePieces.length)];
        console.log(`🤖 AI: Endgame variety - choosing ${choice}`);
        return choice;
      }
      
      // 4. Add some strategic variety (10% chance)
      if (Math.random() < 0.1) {
        const varietyChoices = ['r', 'b', 'n'];
        choice = varietyChoices[Math.floor(Math.random() * varietyChoices.length)];
        console.log(`🤖 AI: Strategic variety - choosing ${choice} for surprise`);
        return choice;
      }
      
    } catch (error) {
      console.log('🤖 AI: Error in promotion analysis, defaulting to engine choice');
    }
    
    // Default to engine's choice (usually queen)
    console.log(`🤖 AI: Standard promotion to ${choice}`);
    return choice;
  };
  ...
  const handlePromotion = (piece: string) => {
    ...
    if (move) {
      ...
      setLastBotMove(null); // Clear bot move highlight when user plays
      // Set user move highlight
      setLastUserMove({ from: pendingPromotion.from, to: pendingPromotion.to });
    }
  ...
  const resetGame = () => {
    ...
    // Reset dynamic stats
    setDynamicAccuracy(0);
    setDynamicElo('0');
    setIsCalculatingStats(false);
  };

  // Function to calculate dynamic accuracy and ELO
  const calculateDynamicStats = async () => {
    setIsCalculatingStats(true);
    try {
      // Get all moves from moveStack
      const allMoves: string[] = [];
      for (const pair of moveStack) {
        if (pair.userMove) allMoves.push(pair.userMove);
        if (pair.aiMove) allMoves.push(pair.aiMove);
      }
      ...
      // Analyze moves to get move types
      const analysisResult = await analyzeMovesLocally(allMoves, Chess, 10);
      
      // Calculate accuracy from analysis (only user moves - even indices)
      const userMoves = analysisResult.analysis.filter((_, index) => index % 2 === 0);
      if (userMoves.length > 0) {
        const moveCounts = userMoves.reduce((acc: any, move: any) => {
          acc[move.type] = (acc[move.type] || 0) + 1;
          return acc;
        }, {});
        
        // Use the pre-calculated accuracy from the analysis result
        setDynamicAccuracy(analysisResult.accuracy);
        setDynamicElo(analysisResult.elo);
        
        // Set brilliance stats
        setBrillianceStats({
          brilliantMoves: moveCounts.Brilliant || 0,
          correctMoves: moveCounts.Correct || 0,
          mistakes: moveCounts.Mistake || 0,
          blunders: moveCounts.Blunder || 0,
        });
      }
    } catch (error) {
      console.error('Error calculating dynamic stats:', error);
      // Fallback to static values
      setDynamicAccuracy(accuracy);
      setDynamicElo(rating.toString());
    } finally {
      setIsCalculatingStats(false);
    }
  };
  ...
  useEffect(() => {
    if (game.isCheckmate()) {
      console.log("Checkmate detected in bot game!");
      setShowCheckmate(true);
      const winnerColor = game.turn() === 'w' ? 'Black' : 'White';
      setWinner(winnerColor);
      setLastGameResult(game.turn() === 'w' ? 'black' : 'white');

      // Calculate dynamic stats when game ends
      calculateDynamicStats();

      // Game saving is now handled in the review page to avoid duplicates

      // Show review after checkmate
      setTimeout(() => {
        setLastGamePgn(game.pgn());
        setShowGameReview(true);
      }, 1200); // Delay to allow checkmate dialog first
    } else {
      // Check for draw conditions
      const drawCheck = checkDrawCondition(game, drawState);
      if (drawCheck.isDraw) {
        console.log("Draw detected in bot game!");
        setShowCheckmate(true);
        setWinner(null);
        setLastGameResult(null);
  ...
  const customSquareStyles = {
    ...
    ...(lastBotMove
      ? {
          [lastBotMove.from]: { background: 'rgba(255,255,0,0.7)' }, // yellow
          [lastBotMove.to]: { background: 'rgba(0,255,0,0.5)' }, // yellow
        }
      : {}),
    // Highlight user's last move
    ...(lastUserMove
      ? {
          [lastUserMove.from]: { background: 'rgba(255,255,0,0.7)' }, // yellow
          [lastUserMove.to]: { background: 'rgba(0,255,0,0.5)' }, // yellow
        }
      : {}),
  };
    ...
    <main className="min-h-screen bg-[var(--background)] relative overflow-hidden">
      ...
      <div className="max-w-screen-xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-[var(--muted-foreground)]">Challenge the AI that plays like {name}</p>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-[280px_minmax(0,600px)_280px] gap-8">
          {/* Bot Message Panel */}
          <div>
            <BotMessagePanel
              botName={name}
              messages={botMessages}
              className="bot-message-panel bg-[var(--card)] text-[var(--primary-text)] shadow-lg"
            />
          </div>

          {/* Chess Board */}
          <div className="flex justify-center items-center w-full max-w-[600px] mx-auto">
            <Card className="bg-[var(--card)] border-[#00F5D4]/30 shadow-lg">
          ...
          <div>
            <MoveHistory
              moves={moveHistory}
              className="move-history-panel bg-[var(--card)] text-[var(--primary-text)] shadow-lg"
            />
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-8">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-[var(--accent)]">{name}</h1>
            <p className="text-[var(--secondary-text)]">{description}</p>
          </div>
          <div className="flex items-center gap-4 mt-4 md:mt-0">
            <div className="text-right">
              <div className="text-sm text-[var(--secondary-text)]">Depth</div>
              <div className="text-lg font-semibold text-[var(--accent)]">{depth}</div>
            </div>
            <div className="text-right">
              <div className="text-sm text-[var(--secondary-text)]">Skill Level</div>
              <div className="text-lg font-semibold text-[var(--accent)]">{skill}</div>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col lg:flex-row justify-center gap-8">
          <div className="flex gap-2 justify-center mt-4 w-full">
            ...
            <Button className="bg-[var(--accent)] text-[var(--card-foreground)] border border-[var(--border)] hover:bg-[#B5532A] btn-glow" onClick={resetGame}>New Game</Button>
            <Button
              className="bg-[var(--accent)] text-[var(--card-foreground)] border border-[var(--border)] hover:bg-[#B5532A] btn-glow"
              variant="outline"
        ...
        {showAnalysis && (
          <div className="flex flex-col items-center">
            <div className="flex items-center gap-2 mb-2">
              ...
              <label htmlFor="voice-toggle" className="text-[var(--primary-text)] select-none">Enable Voice</label>
            </div>
            <div className="flex justify-center lg:justify-start mt-4">
              <Card className="w-full max-w-md bg-[var(--card)] border border-[#00F5D4]/30 shadow-lg">
                ...
                <CardContent>
                  {isAnalyzing ? (
                    <div className="flex items-center justify-center py-4">
                      ...
                      <span className="ml-2 text-[var(--muted-foreground)]">Analyzing position...</span>
                    </div>
                  ) : (
                    <div className="space-y-2">
                      <pre id="position-analysis-result" className="text-sm text-[var(--primary-text)] whitespace-pre-wrap font-sans">
                        {analysisResult}
                      </pre>
                      <Button
                        variant="outline"
                        ...
                        className="mt-3 w-full border-[#818cf8] text-[#818cf8] hover:bg-[#818cf8] hover:text-[var(--card-foreground)]"
                      >
                        Close Analysis
                      </Button>
                    </div>
      ...
      <BrillianceSummaryPopup
        ...
        onAnalyzeGame={() => {
          setShowCheckmate(false);
          // Prepare moves array for review page
          const moves = [];
          for (const m of moveHistory) {
            if (m.userMove) moves.push(m.userMove);
            if (m.aiMove) moves.push(m.aiMove);
          }
          router.push(`/review?moves=${encodeURIComponent(JSON.stringify(moves))}&result=${lastGameResult}`);
        }}
        onPlayAgain={() => {
          setShowCheckmate(false);
          resetGame();
      ...
      {/* Remove the GameReview modal logic below */}
      {/*
      {showGameReview && moveHistory && (
        <GameReview
          moveHistory={(() => { const moves = []; for (const m of moveHistory) { if (m.userMove) moves.push(m.userMove); if (m.aiMove) moves.push(m.aiMove); } return moves; })()}
          onClose={() => setShowGameReview(false)}
          shouldSave={!analyzeUsed}
          gameResult={lastGameResult === 'white' ? 'win' : lastGameResult === 'black' ? 'loss' : 'draw'}
          playerColor="white"
        />
      )}
      */}
      ...
      <Dialog open={!!pendingPromotion} onOpenChange={(open) => { if (!open) setPendingPromotion(null); }}>
        <DialogContent className="max-w-lg w-full rounded-2xl p-8 bg-[var(--card)] border border-[#00F5D4]/30 text-center">
          <DialogHeader>
