...
import { generatePuzzle } from '../../puzzle_logic/engine/puzzleGenerator';
import presetFENs from '../../puzzle_logic/data/presetFENs.json';
...
export default function PuzzlesPage() {
  ...
  const loadPuzzle = async (category: string) => {
    ...
    setError(null)
    try {
      // Get FENs for the selected category
      const fenList = presetFENs[category] || presetFENs["tactics"] || [];
      if (!fenList || fenList.length === 0) {
        throw new Error("No puzzles available for this category");
      }
      ...
      setCurrentPuzzle({
        ...
        theme: category,
        difficulty: 1200, // Placeholder, can be improved
        rating: 1200, // Placeholder, can be improved
        description: `Find the best move for ${puzzle.fen.includes(' w ') ? 'White' : 'Black'}`,
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-gray-400">
            Master chess with AI-powered puzzle training
          </p>
        </div>
                    ...
                    {isLoading ? (
                      ...
                      <div className="aspect-square bg-purple-900/20 rounded-lg flex items-center justify-center">
                        <div className="text-center">
                          ...
                          <Button className="mt-4" onClick={() => loadPuzzle(activeCategory)}>Try Again</Button>
                        </div>
                      </div>
                    ) : currentPuzzle ? (
                      <PuzzleBoard
                        ...
                        onComplete={() => handlePuzzleSolved(true, 0)}
                        onFail={() => handlePuzzleSolved(false, 0)}
                      />
                    ) : null}
                  </CardContent>
                </Card>
