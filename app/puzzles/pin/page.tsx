...
export default function PuzzlesPage() {
  const [currentIndex, setCurrentIndex] = useState(() => {
    if (typeof window !== 'undefined') {
      const savedIndex = localStorage.getItem('pinPuzzleIndex');
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem('pinPuzzleIndex', currentIndex.toString());
    }
  }, [currentIndex]);

  // Fetch total puzzle count on mount
  useEffect(() => {
    fetch('/api/puzzles/pin/count')
      .then(res => res.json())
      .then(data => setTotalPuzzles(data.count))
      .catch(() => setTotalPuzzles(7))
  }, [])
  ...
  const loadPuzzle = async (idx: number) => {
    ...
    setError(null)
    try {
      const response = await fetch(`/api/puzzles/pin/by-index/${idx}`)
  ...
  const handleNextPuzzle = () => {
    ...
    if (token && puzzle && puzzle.fen) {
      fetch(`${backendUrl}/api/users/solve-puzzle`, {
        ...
        body: JSON.stringify({ puzzle_id: puzzle.fen, category: 'Pin' }),
      });
    }
    if (currentIndex + 1 < totalPuzzles) {
      ...
      setCompleted(true)
      toast.success('You solved all the pin puzzles! Explore more parts of this website.')
    }
  }
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-4 mb-4">
            <Link href="/puzzles" className="flex items-center gap-2 text-blue-300 hover:text-blue-200 transition-colors duration-300">
          ...
          <div className="flex items-center gap-3 mb-2">
            <div>
              ...
              <p className="text-[var(--muted-foreground)] mt-1">Practice pin tactics to win material or create threats</p>
            </div>
          </div>
        </div>
            ...
            {loading ? (
              ...
              <div className="glass-effect rounded-xl p-8 border border-[var(--border)]/10">
                <div className="flex flex-col items-center justify-center h-96 gap-4 text-center">
                  ...
                  <Button 
                    onClick={handleRetry} 
                    className="bg-gradient-to-r from-blue-500 to-cyan-500 hover:from-blue-600 hover:to-cyan-600 text-[var(--card-foreground)] rounded-lg px-6 py-2 font-bold shadow-lg hover:shadow-blue-500/25 transition-all duration-300"
                  >
              ...
              <div className="glass-effect rounded-xl p-8 border border-[var(--border)]/10">
                <div className="flex flex-col items-center justify-center h-96 gap-6 text-center">
                  ...
                  <Button 
                    ...
                    className="bg-gradient-to-r from-blue-500 to-cyan-500 hover:from-blue-600 hover:to-cyan-600 text-[var(--card-foreground)] rounded-lg px-8 py-3 font-bold shadow-lg hover:shadow-blue-500/25 transition-all duration-300"
                  >
                    Restart Puzzles
                  </Button>
                </div>
          ...
          <div className="lg:col-span-1">
            <div className="glass-effect rounded-xl p-6 border border-[var(--border)]/10 mb-6">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <h3 className="text-lg font-bold text-[var(--card-foreground)]">Pin Tactics</h3>
                  <Badge className="bg-[var(--accent)]/20 text-green-400 border-green-500/30 mt-1">Beginner</Badge>
                </div>
              </div>
              <p className="text-[var(--muted-foreground)] text-sm mb-4">
                Practice pin tactics to win material or create threats. Pin your opponent's pieces to gain advantage.
              </p>
              <div className="flex items-center gap-2 text-[var(--muted-foreground)] mb-3">
                <span className="text-sm font-medium">PIN TACTICS</span>
            ...
            {showExplanation && (
              <div className="glass-effect rounded-xl p-6 border border-[var(--border)]/10 mb-6">
                ...
                <p className="text-[var(--muted-foreground)] text-sm mb-3">
                  The best move is: <span className="font-mono text-blue-400">{solutionMove}</span>
                </p>
                <p className="text-[var(--muted-foreground)] text-xs">
                  This move creates a pin that restricts the opponent's piece movement and gives you a tactical advantage.
                </p>
              </div>
            )}
