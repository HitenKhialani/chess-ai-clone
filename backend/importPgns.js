  ...
  for (const filename of pgnFiles) {
    ...
    try {
      ...
      for (let i = 0; i < Math.min(games.length, 5); i++) {
        ...
        let parsed;
        try {
          parsed = pgnParser.parse(gameContent);
        } catch (err) {
          console.error(`PGN from ${filename} (game ${i + 1}) failed to parse with @mliebelt/pgn-parser and will be skipped.`, err);
          continue;
        }
        if (!parsed || parsed.length === 0) {
          console.error(`PGN from ${filename} (game ${i + 1}) failed to parse with @mliebelt/pgn-parser (no games found) and will be skipped.`);
          continue;
        }
        const chess = new Chess();
        const loaded = chess.loadPgn(gameContent);
        if (!loaded) {
          console.error(`PGN from ${filename} (game ${i + 1}) is invalid for chess.js and will be skipped.`);
          continue;
        }
