...
router.post('/save-game', authMiddleware, (req, res) => {
  ...
  try {
    // Extract just the moves from the game report for duplicate detection
    const moves = Array.isArray(game_report) ? game_report.map(move => move.move || move) : [];
    const movesString = JSON.stringify(moves);
    
    // Check for existing game with same moves and result (more comprehensive duplicate detection)
    
    // First check: Look for games with same first and last moves (quick check)
    const existing = db.prepare(`
      SELECT id, game_report FROM user_games 
      WHERE user_id = ? AND result = ? 
      AND game_report LIKE ?
    `).get(userId, result, `%${moves[0]}%${moves[moves.length-1]}%`);
    
    if (existing) {
      // Second check: Verify the exact move sequence to prevent false positives
      try {
        const existingGameReport = JSON.parse(existing.game_report);
        const existingMoves = existingGameReport.map(move => move.move || move);
        
        if (existingMoves.length === moves.length && 
            JSON.stringify(existingMoves) === movesString) {
          console.log('Exact duplicate game detected, skipping save');
          return res.status(200).json({ msg: 'Game report already saved' });
        }
      } catch (parseError) {
        console.log('Error parsing existing game report, proceeding with save');
      }
    }
...
// POST /api/users/award-best-moves
// Award coins for best moves at odd positions (1,3,5,...)
router.post('/award-best-moves', authMiddleware, (req, res) => {
  const { game_report } = req.body;
  const userId = req.user.id;
  
  if (!game_report || !Array.isArray(game_report)) {
    return res.status(400).json({ msg: 'Missing or invalid game report' });
  }
  
  try {
    // Debug: Log the game report to understand the structure
    console.log('Game report received:', JSON.stringify(game_report, null, 2));
    
    // Count best moves at odd positions (1,3,5,...)
    // Based on the move history, it seems the array contains only player moves
    // So we need to award coins for best moves at odd positions: 1, 3, 5, 7, 9, 11, 13, 15, 17, etc.
    const bestMovesAtOddPositions = game_report.filter((move, idx) => {
      const isBest = move.type === 'Best';
      const gamePosition = idx + 1; // Convert 0-indexed to 1-indexed
      const isOddPosition = gamePosition % 2 === 1; // Odd positions: 1, 3, 5, 7, 9, 11, 13, 15, 17, etc.
      console.log(`Move ${gamePosition} (idx ${idx}): type=${move.type}, isBest=${isBest}, isOddPosition=${isOddPosition}, move=${move.move}`);
      return isBest && isOddPosition;
    }).length;
    
    if (bestMovesAtOddPositions > 0) {
      // Award 1 coin for each best move at odd position
      db.prepare('UPDATE users SET coins = COALESCE(coins, 0) + ? WHERE id = ?').run(bestMovesAtOddPositions, userId);
      
      res.json({ 
        msg: 'Coins awarded for best moves', 
        coinsAwarded: bestMovesAtOddPositions,
        totalBestMovesAtOddPositions: bestMovesAtOddPositions
      });
    } else {
      res.json({ 
        msg: 'No best moves at odd positions to award', 
        coinsAwarded: 0,
        totalBestMovesAtOddPositions: 0
      });
    }
  } catch (err) {
    console.error(err.message);
    res.status(500).json({ msg: 'Failed to award coins for best moves' });
  }
});
