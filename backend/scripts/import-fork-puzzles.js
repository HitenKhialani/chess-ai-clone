...
const createTableSQL = fs.readFileSync(path.join(__dirname, 'create-fork-puzzles-table.sql'), 'utf8');
db.exec(createTableSQL);

// Accept PGN filename as a command-line argument, default to fork_puzzles_5.pgn
const inputFile = process.argv[2]
  ? path.resolve(process.cwd(), process.argv[2])
  : path.join(__dirname, '../../fork_puzzles_5.pgn');
...
for (const block of puzzleBlocks) {
  ...
  if (fen && moveLine) {
    ...
    db.prepare(`INSERT INTO fork_puzzles (fen, moves, themes, rating, source) VALUES (?, ?, ?, ?, ?)`)
      .run(fen, moves, themes, rating || null, source);
    count++;
  }
}

console.log(`Imported ${count} fork puzzles from PGN.`); 