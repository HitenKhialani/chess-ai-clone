...
// Accept PGN filename as a command-line argument, default to mate_in_2_puzzles_7.pgn at repo root
const inputFile = process.argv[2]
  ? path.resolve(process.cwd(), process.argv[2])
  : path.join(__dirname, '../../mate_in_2_puzzles_7.pgn');

if (!fs.existsSync(inputFile)) {
  console.warn(`Mate-in-2 PGN file not found at ${inputFile}. Skipping import.`);
  process.exit(0);
}
...
for (const block of puzzleBlocks) {
  ...
  const themes = 'Mate in 2';
  const rating = 1200; // default rating for mate-in-2 puzzles
...
console.log(`Imported ${count} mate-in-2 puzzles from PGN.`);
