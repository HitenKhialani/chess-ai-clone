...
// Accept PGN filename as a command-line argument, default to pin_puzzles_short_1_to_4_moves.pgn
const inputFile = process.argv[2]
  ? path.resolve(process.cwd(), process.argv[2])
  : path.join(__dirname, '../../pin_puzzles_short_1_to_4_moves.pgn');
...
for (const block of puzzleBlocks) {
  ...
  if (fen && moveLine) {
    ...
    db.prepare(`INSERT INTO pin_puzzles (fen, moves, themes, rating, source) VALUES (?, ?, ?, ?, ?)`)
      .run(fen, moves, themes, rating || null, source);
    count++;
  }
}

console.log(`Imported ${count} pin puzzles from PGN.`); 