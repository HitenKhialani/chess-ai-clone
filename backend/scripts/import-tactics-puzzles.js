...
const createTableSQL = fs.readFileSync(path.join(__dirname, 'create-tactics-puzzles-table.sql'), 'utf8');
db.exec(createTableSQL);

// Accept PGN filename as a command-line argument, default to tactical_puzzle_7.pgn
const inputFile = process.argv[2]
  ? path.resolve(process.cwd(), process.argv[2])
  : path.join(__dirname, '../../tactical_puzzles_7.pgn');
...
for (const block of puzzleBlocks) {
  ...
  if (fen && moveLine) {
    ...
    db.prepare(`INSERT INTO tactics_puzzles (fen, moves, themes, rating, source) VALUES (?, ?, ?, ?, ?)`)
      .run(fen, moves, themes, rating || null, source);
    count++;
  }
}

console.log(`Imported ${count} tactics puzzles from PGN.`); 