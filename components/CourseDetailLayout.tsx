            ...
            <div key={i} className="mb-6">
              <div className="flex items-center justify-between cursor-pointer" onClick={() => setExpanded(expanded === i ? -1 : i)}>
                ...
                <div className="flex gap-2 items-center">
                  ...
                  <span className={`ml-2 px-2 py-1 rounded text-xs font-semibold ${chapter.isCompleted ? 'bg-[var(--accent)] text-[var(--accent-foreground)]' : 'bg-[var(--accent)]/20 text-[var(--accent)]'}`}>{chapter.isCompleted ? 'Completed' : 'In Progress'}</span>
                </div>
              </div>
              {expanded === i && (
