...
export const GameReviewDisplay: React.FC<GameReviewDisplayProps> = ({ review, summary, accuracy }) => {
  ...
  return (
    <div className="w-full max-w-lg mx-auto p-4">
      {/* Summary bubble */}
      <div className="bg-[#f6edff] rounded-lg p-3 mb-2 flex items-center gap-3">
        <img src={placeholderAvatar} alt="You" className="w-10 h-10 rounded-full border-2 border-[#7C3AED]" />
        <span className="text-gray-700 text-sm">{summary || 'Some good positional play let you seize the advantage in the middlegame.'}</span>
      </div>
      {/* Evaluation graph */}
      ...
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          <img src={placeholderAvatar} alt="You" className="w-10 h-10 rounded-full border-2 border-[#7C3AED]" />
          <div>
            <div className="font-bold text-[#7C3AED]">You</div>
            <div className="text-xs text-gray-500">Accuracy</div>
            <div className="font-bold text-lg text-[#7C3AED]">{computedAccuracy}</div>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <img src={botAvatar} alt="Bot" className="w-10 h-10 rounded-full border-2 border-[#818cf8]" />
          <div>
            <div className="font-bold text-[#818cf8]">Bot</div>
            <div className="text-xs text-gray-500">Accuracy</div>
            <div className="font-bold text-lg text-[#818cf8]">-</div>
          </div>
        </div>
      </div>
      {/* Move type summary */}
      <div className="grid grid-cols-6 gap-2 text-center mb-2">
        {['Brilliant', 'Great', 'Best', 'Mistake', 'Miss', 'Blunder'].map((type) => (
          <div key={type} className="flex flex-col items-center">
            ...
            <span className={`font-bold ${typeColors[type] || ''}`}>{moveTypeCounts[type] || 0}</span>
          </div>
        ))}
      </div>
    </div>
