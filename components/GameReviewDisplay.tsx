...
const typeColors: Record<string, string> = {
  ...
  Mistake: 'text-yellow-500',
  Blunder: 'text-[var(--destructive)]',
};
...
export const GameReviewDisplay: React.FC<GameReviewDisplayProps> = ({ review, summary, accuracy }) => {
  ...
  const computedAccuracy = accuracy || (review.length
    ? (
        ((moveTypeCounts['Brilliant'] || 0) + (moveTypeCounts['Correct'] || 0) - 
         ((moveTypeCounts['Mistake'] || 0) * 0.5) - ((moveTypeCounts['Blunder'] || 0) * 1)) / review.length * 100
      ).toFixed(1)
    : '0.0');
  ...
  return (
    <div className="game-review-panel bg-[var(--card)] text-[var(--primary-text)] shadow-xl rounded-2xl p-6 font-sans relative">
      ...
      <button
        className="md:hidden absolute top-2 right-2 z-20 bg-[var(--card)]/80 rounded-full shadow p-2 border border-[var(--border)]"
      ...
      <div className="bg-[#f6edff] rounded-lg p-3 mb-2 flex items-center gap-3 shadow-sm">
        ...
        <span className="text-[var(--card-foreground)] text-sm font-sans">{summary || 'Some good positional play let you seize the advantage in the middlegame.'}</span>
      </div>
      {/* Evaluation graph */}
      <div className="bg-[var(--card)] rounded-lg shadow p-2 mb-2">
