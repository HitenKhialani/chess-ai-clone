...
export function BotMessagePanel({ botName, messages }: BotMessagePanelProps) {
  return (
    <Card className="w-full h-[400px] bg-[var(--card)] border-[var(--border)]">
      <CardContent className="p-4">
        <div className="flex items-center gap-2 mb-4">
          <Bot className="w-5 h-5 text-[var(--accent)]" />
          <h3 className="font-semibold text-[var(--accent)]">{botName}'s Thoughts</h3>
        </div>
        <ScrollArea className="h-[320px] pr-4">
          <div className="space-y-4">
            {messages.length === 0 ? (
              <div className="bg-[var(--background)] p-3 rounded-lg">
                <p className="text-sm text-[var(--primary-text)]">I'm ready to judge your moves! Make your first move and watch for my comments every few turns. 😏</p>
              </div>
            ) : (
              messages.map((message) => (
                <div key={message.id} className="bg-[var(--background)] p-3 rounded-lg">
                  <p className="text-sm text-[var(--primary-text)]">{message.text}</p>
                  <span className="text-xs text-[var(--secondary-text)] mt-1 block">
