...
export function ChapterDetail({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={() => onClose()}>
      <DialogContent className="max-w-4xl bg-[var(--background)] text-[var(--card-foreground)]">
        <DialogHeader>
          <DialogTitle className="text-2xl font-bold text-[var(--card-foreground)]">{title}</DialogTitle>
          <p className="text-[var(--muted-foreground)] mt-2">{description}</p>
        </DialogHeader>
            ...
            <div>
              ...
              <div className="space-y-2">
                {content.theory.map((point, index) => (
                  <p key={index} className="text-[var(--muted-foreground)]">• {point}</p>
                ))}
              </div>
            </div>
