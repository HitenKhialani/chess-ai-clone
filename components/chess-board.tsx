...
function ChessBoard({ fen, solution = [], onSolved }: ChessBoardProps) {
  ...
  return (
    <div className="space-y-4 chess-board-container" style={{ fontSize: '1rem', lineHeight: '1.5' }}>
      {/* Game State Indicator */}
      {gameState !== "playing" && (
        <div
          className={`p-4 rounded-lg text-center ${
            gameState === "correct"
              ? "bg-[var(--accent)]/20 border border-green-600/50"
              : "bg-[var(--destructive)]/20 border border-red-600/50"
          }`}
          style={{ fontSize: '1rem' }}
        >
      ...
      {/* Chess Board */}
      <div className="aspect-square max-w-lg mx-auto chess-board" style={{ fontSize: '1rem', lineHeight: '1.5' }}>
          ...
          {board.map((row, rowIndex) =>
            row.map((piece, colIndex) => {
              ...
              return (
                ...
                >
                  <span
                    className={`select-none ${piece && piece === piece.toUpperCase() ? "text-[var(--accent-foreground)]" : "text-black"}`}
                  >
      ...
      <div className="flex justify-center space-x-4">
        <Button
          onClick={resetPuzzle}
          variant="outline"
          className="border-purple-600 text-purple-200 hover:bg-[var(--accent)]/20"
          style={{ fontSize: '1rem' }}
        >
      ...
      <div className="text-center" style={{ fontSize: '1rem' }}>
        <p className="text-purple-200 text-sm">
          Move {moveIndex + 1} of {solution ? solution.length : 0}
          {moveIndex === 0 && solution && solution.length > 0 && (
            <span className="block mt-1 text-purple-300">
              Hint: Look for {solution[0] && solution[0].length > 2 ? "a tactical shot" : "the best move"}
            </span>
          )}
        </p>
      </div>
