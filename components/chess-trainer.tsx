...
export default function ChessTrainer() {
  ...
  const onDrop = (sourceSquare: string, targetSquare: string) => {
    ...
    if (isPromotion) {
      // Verify this is a legal move before showing promotion dialog
      const moves = game.moves({ verbose: true });
      const isLegalMove = moves.some(m => m.from === sourceSquare && m.to === targetSquare);
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      {drawInfo.isDraw && (
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black/40">
          <div className="bg-[var(--card)] rounded-xl shadow-xl p-8 text-center">
