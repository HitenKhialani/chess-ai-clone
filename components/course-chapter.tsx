...
export function CourseChapter({
  ...
  return (
    <>
      <Card className={`bg-[var(--card)] border-gray-700 ${isLocked ? 'opacity-75' : ''}`}>
        <CardContent className="p-6">
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-2">
                <h3 className="font-bold text-lg text-[var(--card-foreground)]">{title}</h3>
      ...
      {!isLocked && positions.length > 0 && (
        <div className="mt-6 space-y-6">
          <div className="flex items-center justify-between">
            <h4 className="font-medium text-[var(--card-foreground)]">Practice Position {currentPositionIndex + 1}/{positions.length}</h4>
          ...
          <div className="bg-[var(--background)] rounded-lg p-4">
            <p className="text-[var(--muted-foreground)] mb-4">{positions[currentPositionIndex].explanation}</p>
            <PuzzleBoard
