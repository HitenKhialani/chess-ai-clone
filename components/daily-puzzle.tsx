  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-purple-800 to-indigo-900 p-8">
      <div className="container mx-auto">
        <Button onClick={onBack} variant="ghost" className="mb-6 text-purple-200 hover:text-[var(--card-foreground)]">
        ...
        <div className="text-center mb-8">
          <div className="flex items-center justify-center space-x-3 mb-4">
            ...
            <h1 className="text-4xl font-bold text-[var(--card-foreground)]">Daily Puzzle</h1>
          </div>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
            ...
            <Card className="bg-black/30 border-purple-700/50">
              <CardHeader>
                <div className="flex items-center justify-between">
                  <CardTitle className="text-[var(--card-foreground)] flex items-center space-x-2">
                    <Star className="h-5 w-5 text-indigo-400" />
                    <span>Daily Challenge</span>
                  </CardTitle>
                  <Badge variant="secondary" className="bg-[var(--accent)]/20 text-indigo-200">
              ...
              <CardContent>
                {solved ? (
                  <div className="space-y-4">
                    <div className="bg-[var(--accent)]/20 border border-green-600/50 p-6 rounded-lg text-center">
          ...
          <div className="space-y-6">
            ...
            <Card className="bg-black/30 border-purple-700/50">
              <CardHeader>
                <CardTitle className="text-[var(--card-foreground)]">Puzzle Details</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <div>
                  <p className="text-purple-200 mb-3">{todaysPuzzle.description}</p>
                  <div className="flex flex-wrap gap-2">
                    <Badge variant="secondary" className="bg-[var(--accent)]/20 text-purple-200">
                      {todaysPuzzle.theme}
                    </Badge>
                    <Badge variant="secondary" className="bg-[var(--primary)]/20 text-blue-200">
                      {todaysPuzzle.difficulty} Elo
                    </Badge>
                  </div>
                </div>
              ...
              <CardContent className="space-y-4">
                <div className="space-y-3">
                  <div className="flex justify-between">
                    ...
                    <span className="text-[var(--card-foreground)] font-bold">{todaysPuzzle.solvers.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-purple-200">Average Time</span>
                    <span className="text-[var(--card-foreground)] font-bold">{formatTime(todaysPuzzle.averageTime * 1000)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-purple-200">Your Attempts</span>
                    <span className="text-[var(--card-foreground)] font-bold">{attempts}</span>
                  </div>
                </div>
            ...
            <Card className="bg-black/30 border-purple-700/50">
              <CardHeader>
                <CardTitle className="text-[var(--card-foreground)]">Daily Streak</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="text-center">
                  ...
                  <div className="flex justify-center space-x-1">
                    {[...Array(7)].map((_, i) => (
                      <div key={i} className={`w-6 h-6 rounded-full ${i < 7 ? "bg-[var(--accent)]" : "bg-gray-600"}`} />
                    ))}
                  </div>
