...
export function PuzzleTimer({ difficulty, onTimeUp }: PuzzleTimerProps) {
  ...
  return (
    <div className="flex items-center space-x-3">
              <Clock className={`h-5 w-5 ${isUrgent ? "text-red-400" : "text-[#00F5D4]"}`} />
      <div className="flex items-center space-x-2">
        <span className={`font-mono font-bold ${isUrgent ? "text-red-400" : "text-[var(--accent-foreground)]"}`}>
          {formatTime(timeLeft)}
        </span>
        <div className="w-20">
          <Progress value={progressPercentage} className={`h-2 ${isUrgent ? "bg-red-900" : "bg-[#00F5D4]/20"}`} />
        </div>
      </div>
    </div>
  )
