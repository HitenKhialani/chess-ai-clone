...
export function ChessBoard({ fen, solution, onSolved }: ChessBoardProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Chess Board */}
      <div className="aspect-square max-w-lg mx-auto animate-float-slow">
        <div className="grid grid-cols-8 gap-0 border-2 border-[#00F5D4]/50 rounded-lg overflow-hidden animate-rotate-slow">
              ...
              return (
                <div
                  ...
                  className={`
                    ...
                    ${isSelected ? "ring-4 ring-[#00F5D4]" : ""}
                    ${isLastMove ? "bg-green-400/50" : ""}
                  `}
                  onClick={() => handleSquareClick(rowIndex, colIndex)}
                >
                  <span
                    className={`select-none chess-piece-floating ${piece && piece === piece.toUpperCase() ? "text-white" : "text-black"}`}
                  >
      ...
      <div className="flex justify-center space-x-4">
        <Button
          onClick={resetPuzzle}
          variant="outline"
                      className="border-[#00F5D4] text-[#00F5D4] hover:bg-[#00F5D4]/20"
        >
